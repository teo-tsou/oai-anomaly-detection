...
def generate_packet(row):
    ...
    if protocol == 'tcp':
        inner_packet = IP(src= "12.2.1.3", dst="12.2.1.1")/TCP(dport=port, flags=flag)/Raw(load=src_payload)
    elif protocol == 'udp':
        inner_packet = IP(src= "12.2.1.3", dst="12.2.1.1")/UDP(dport=port)/Raw(load=src_payload)
    else:
        inner_packet = IP(src= "12.2.1.3", dst="12.2.1.1")/ICMP()  # Default for other protocols
    ...
    if dst_bytes > 0:
        ...
        response_inner_packet = IP(src="12.2.1.1", dst="12.2.1.3")/TCP(sport=port, dport=1024, flags='PA')/Raw(load=dst_payload)
        # Encapsulate response in GTP-U
        response_gtp_packet = IP(src= "12.2.1.3", dst="12.2.1.1")/UDP(sport=2152, dport=2152)/GTP_U_Header(teid=1)/response_inner_packet
